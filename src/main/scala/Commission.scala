        ...
        case "Delivery" =>
          ...
          val managerTransactions = for {
            ...
            order <- etsyOrders
              .get(orderId)
              .toRight(
                "No corresponding order found for this commission transaction in orders csv file."
              )
            shippingRevenue = order.deliveryValue
            commissionFee <- validateCommission(net, shippingRevenue)
          } yield List(
            ManagerTransaction(date, descriptionPrefix + info, commissionFee)
          )
          managerTransactions.mapLeft(err =>
            s"Could not process Commission transaction. $err Commission transaction: ${this.toString}"
          )
        case _ =>
          ...
          val managerTransactions = for {
            ...
            item <- etsySoldItems
              .get(transactionId)
              .toRight(
                "No corresponding sold item found for this commission transaction in sold items csv file."
              )
            ...
            ManagerTransaction(
              date,
              descriptionPrefix + info + " order: " + orderId,
              commissionFee
            )
          )
          managerTransactions.mapLeft(err =>

...
case class Refund(
  ...
  // the price though. $8.50 or $25 almost certainly means a shipping refund, so we'll put this down as shipping refund,
  // else sales refund.
  override def toManagerTransactions(
    ...
    Right(
      List(
        ManagerTransaction(
          date,
          if (amount == BigDecimal(-8.5)) "Shipping refund: " + title
          else "Sale refund: " + title,
          amount
        ),
        ManagerTransaction(
          date,
          s"Card processing fees refund: $title",
          feesAndTaxes
        )
      )
    )

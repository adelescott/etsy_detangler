...
case class Sale(
  ...
  def validateTaxCollected(
    ...
    if (taxCollected == taxRemitted)
      ...
      Left("Could not validate the tax collected. Appears as though the tax collected is not equal to the tax " +
  ...
  override def toManagerTransactions(
    ...
    managerTransactions.mapLeft(
      err => s"Could not process Sale transaction. $err Sale transaction: ${this.toString}"
    )
  }
}
